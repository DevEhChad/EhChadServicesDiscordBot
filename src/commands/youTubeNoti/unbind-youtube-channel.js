...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (!doc || !doc.channelId) return interaction.reply({ content: 'No YouTube notification channel is currently set for this server.', ephemeral: true });

    doc.channelId = null;
    await doc.save();
    return interaction.reply({ content: '✅ Removed the YouTube notification channel. Use `/bind-youtube-channel` to set a new one.', ephemeral: true });
  }
};