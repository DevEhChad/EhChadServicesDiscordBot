...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt.setName('target').setDescription('Command or service name (or "all")').setRequired(false)
    ),
  ...
  async execute(interaction) {
    try {
      ...
      if (!devs.includes(interaction.user.id)) {
        return interaction.reply({ content: 'This command is restricted to the bot developers.', flags: MessageFlags.Ephemeral });
      }
      ...
      if (!target) {
        ...
        if (options.length === 0) return interaction.reply({ content: 'No commands or services available to manage.', flags: MessageFlags.Ephemeral });
      ...
      if (!['command', 'service'].includes(type)) {
        return interaction.reply({ content: 'Invalid target type. Must be "command" or "service".', flags: MessageFlags.Ephemeral });
      }
      // If target provided, toggle or display status
      const entry = await Toggle.findOne({ key, type, guildId: null });
      if (!entry) {
        return interaction.reply({ content: `No toggle configured for ${target}. Use the interactive view to create one.`, flags: MessageFlags.Ephemeral });
      }
      return interaction.reply({ content: `Toggle for ${target}: ${entry.enabled ? 'enabled' : 'disabled'} (devOnly: ${entry.devOnly})`, flags: MessageFlags.Ephemeral });
    } catch (error) {
